...
def embed_in_image(image_path: str, data_path: str, output_path: str) -> None:
    ...
    try:
        ...
        output_name = f"stega_{timestamp}.png"
...
def extract_from_image(image_path: str, output_path: str) -> None:
    ...
    try:
        ...
        extracted_bits = ''
        total_bits = 0
        max_bits = len(pixels) * 3  # Maximum possible bits
        
        # First extract enough bits for the header
        for r, g, b in pixels:
            ...
            if total_bits >= len(MAGIC_MARKER) * 8 + 32:
                break
        ...
        # Extract more bits if needed for data length
        ...
        if data_length * 8 > max_bits - current_pos:
            raise SteganographyError("Data length exceeds available bits")
        
        # Extract remaining bits for data
        needed_bits = current_pos + (data_length * 8)
        for pixel_index in range(len(extracted_bits) // 3, (needed_bits + 2) // 3):
            if pixel_index >= len(pixels):
                break
            r, g, b = pixels[pixel_index]
            extracted_bits += str(r & 1)
            ...
            if pixel_index % 100000 == 0:
                progress = (len(extracted_bits) / needed_bits) * 100
